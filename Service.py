...
def apply_blessing(engine, hero):
    if hero.gold >= int(20 * 1.5**engine.level) - 2 * hero.stats["intelligence"]:
        ...
        if random.randint(0, 1) == 0:
            ...
            engine.notify("Your hero has been blessed")
        else:
            engine.hero = Objects.Berserk(hero)
            engine.notify("You are the berserk!")
    else:
        engine.score -= 0.1
...
def remove_effect(engine, hero):
    if hero.gold >= int(10 * 1.5**engine.level) - 2 * hero.stats["intelligence"] and "base" in dir(hero):
        ...
        engine.notify("An effect has been removed")


def apply_curse(engine, hero):
    s = random.randint(0, 2*hero.stats['luck'])
    if s < hero.stats['luck']:
        hero.stats['endurance'] += 2
        restore_hp(engine, hero)
    else:
        hero.hp -= int(hero.hp/2)
        engine.hero = Objects.Curse(hero)
        engine.notify("You are cursed!")
...
def add_gold(engine, hero):
    if random.randint(1, 10) == 1:
        ...
        engine.notify("Your Hero is weakening")
    else:
        engine.score += 0.1
        gold = int(random.randint(10, 1000) * (1.1**(engine.hero.level - 1)))
        hero.gold += gold
        engine.notify(f"{gold} gold has been recieved")
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            for obj_name in object_list_prob['objects']:
                prop = object_list_prob['objects'][obj_name]
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(1, 16), random.randint(1, 9))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
                            intersect = True
                            coord = (random.randint(1, 16),
                                     random.randint(1, 9))
                            continue
                        for obj in self.objects:
                            if coord == obj.position or coord == (1, 1):
...
class SpecialMap(MapFactory):
    ...
    class Map:
        ...
        def __init__(self):
            SpecialMap.size_map = len(self.spec_map[SpecialMap.number_map])
            self.Map = self.spec_map[SpecialMap.number_map]
            self.Map = list(map(list, self.Map))
            for i in self.Map:
                for j in range(len(i)):
                    i[j] = wall if i[j] == '0' else [floor1, floor2, floor3][random.randint(0, 2)]
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in object_list_prob['objects']:
                prop = object_list_prob['objects'][obj_name]
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(2, 19), random.randint(2, SpecialMap.size_map-2))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
            ...
            for obj_name in object_list_prob['ally']:
                prop = object_list_prob['ally'][obj_name]
                for i in range(random.randint(prop['min-count'], prop['max-count'])):
                    coord = (random.randint(1, 19), random.randint(1, SpecialMap.size_map-2))
                    intersect = True
                    while intersect:
                        intersect = False
                        if _map[coord[1]][coord[0]] == wall:
...
def service_init(sprite_size, full=True):
    ...
    for ally in object_list_prob['ally']:
        ...
        prop['sprite'][0] = create_sprite(os.path.join(ALLY_TEXTURE, prop_tmp['sprite'][0]), sprite_size)
        prop['action'] = object_list_actions[prop_tmp['action']]
